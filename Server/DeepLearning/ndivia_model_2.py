...
class Model:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope(self.name):
            ...
            # Y : 6가지
            # 100 => 6
            self.logits = tf.layers.dense(inputs=dropout8, units=1)
    ...
    def get_accuracy(self, x_test, y_test, training=False):
        ...
        correct_prediction = tf.equal(self.logits, self.Y)
        accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))


        real_value =  self.sess.run(self.Y, feed_dict={self.Y: y_test, self.training: training})
